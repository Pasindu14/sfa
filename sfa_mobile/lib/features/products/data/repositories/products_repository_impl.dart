...
class ProductsRepositoryImpl implements ProductsRepository {
  ...
  @override
  Future<(List<Product>, DateTime)> syncProducts({bool force = false}) async {
  ...
  Future<(List<Product>, DateTime)> _sync({required bool force}) async {
    final ifNoneMatch = force ? null : await _conditionalEtag();
    final result = await _remote.getProducts(ifNoneMatch: ifNoneMatch);
    ...
    switch (result) {
      ...
      case Fetched(:final data, :final etag):
        ...
        await _tryEtag(() => _etags.clear(EtagStore.products));
        await _local.replaceAll(data.products);
        if (etag != null) {
          await _tryEtag(() => _etags.write(EtagStore.products, etag));
        }
        await _local.saveLastSyncedAt(data.cachedAt);
        return (
          data.products.map((m) => m.toEntity()).toList(),
          data.cachedAt,
        );
    }
  }
  ...
  Future<String?> _conditionalEtag() async {
    try {
      final etag = await _etags.read(EtagStore.products);
