...
class SupervisorNotBillingPage extends StatelessWidget {
  const SupervisorNotBillingPage({super.key});
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          const _OrangeAppBar(),
          Expanded(
            child: BlocBuilder<SupervisorNotBillingBloc,
                SupervisorNotBillingState>(
              builder: (context, state) {
                if (state is SupervisorNotBillingInitial ||
                    state is SupervisorNotBillingLoading) {
                  return const _LoadingBody();
                }
                if (state is SupervisorNotBillingError) {
                  return _ErrorBody(message: state.message);
                }
                if (state is SupervisorNotBillingReady) {
                ...
                children: [
                  ...
                  Column(
                    ...
                    children: [
                      ...
                      SizedBox(height: 2.r),
                      Text(
                        'Non-billing visits by your sales reps',
...
class _ReadyBody extends StatelessWidget {
  final SupervisorNotBillingReady state;
  const _ReadyBody({required this.state});

  @override
  Widget build(BuildContext context) {
    final notBillings =
        state.hasResults ? state.notBillings! : const <NotBillingSummary>[];
    // Slivers so only the visible record cards are built. The form above the
        ...
        SliverPadding(
          ...
          sliver: SliverMainAxisGroup(
            slivers: [
              ...
              if (notBillings.isNotEmpty)
                SliverList.builder(
                  itemCount: notBillings.length,
                  itemBuilder: (context, index) {
                    final nb = notBillings[index];
                    return Align(
                      alignment: AlignmentDirectional.topStart,
                      child: Padding(
                        padding: EdgeInsets.only(bottom: 10.h),
                        child: _NotBillingCard(
                          notBilling: nb,
                          onTap: () => context.push(
                            '/supervisor/not-billing/${nb.id}',
                            extra: nb.notBillingNumber,
                          ),
                        ),
                      ),
                    );
      ...
      children: [
        ...
        _StepCard(
          ...
          child: _SearchableSelectField<RepSummary>(
            ...
            enabled: !state.isLoadingNotBillings,
            onChanged: (rep) {
              if (rep != null) {
                context
                    .read<SupervisorNotBillingBloc>()
        ...
        // ── Error banner ──────────────────────────────────────────────────────
        if (state.notBillingsError != null) ...[
          _ErrorBanner(message: state.notBillingsError!),
          SizedBox(height: 16.h),
        ],

        // ── Get Non-Billings button ───────────────────────────────────────────
        _GetNotBillingsButton(
          canLoad: state.canLoad,
          isLoading: state.isLoadingNotBillings,
          onTap: () => context
              .read<SupervisorNotBillingBloc>()
    ...
    return DraggableScrollableSheet(
      ...
      builder: (_, scrollController) => ClipRRect(
        ...
        child: Container(
          ...
          child: Column(
            children: [
              // Orange gradient header + search field
              Container(
                decoration: BoxDecoration(
                  gradient: LinearGradient(
                    begin: Alignment.topLeft,
...
class _GetNotBillingsButton extends StatelessWidget {
  final bool canLoad;
  final bool isLoading;
  final VoidCallback onTap;

  const _GetNotBillingsButton({
      ...
      children: [
        Row(
          children: [
            ...
            Expanded(
              child: Text(
                '${notBillings.length} RECORD${notBillings.length == 1 ? '' : 'S'} · ${_formatDate(date)}',
                style: GoogleFonts.barlowCondensed(
                  fontSize: 11.sp,
                  fontWeight: FontWeight.w700,
                  letterSpacing: 2.5,
        ...
        SizedBox(height: 12.h),
        // The record cards themselves are a lazy SliverList in _ReadyBody.
        if (notBillings.isEmpty) _EmptyState(repName: repName, date: date),
      ],
    );
  }
}
...
class _EmptyState extends StatelessWidget {
  ...
  const _EmptyState({required this.repName, required this.date});
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          SizedBox(height: 4.h),
          Text(
            '$repName had no non-billings on ${_formatDate(date)}',
            textAlign: TextAlign.center,
            style: GoogleFonts.barlow(
                fontSize: 12.sp,
                color: AppColors.foregroundMuted,
...
class _NotBillingCard extends StatefulWidget {
  final NotBillingSummary notBilling;
  final VoidCallback onTap;

  const _NotBillingCard({required this.notBilling, required this.onTap});

  @override
  State<_NotBillingCard> createState() => _NotBillingCardState();
}

class _NotBillingCardState extends State<_NotBillingCard> {
  bool _pressed = false;
  ...
  _ReasonMeta get _reasonMeta {
    switch (widget.notBilling.reason) {
      case NotBillingReason.outletClosed:
        return _ReasonMeta('Closed', AppColors.warning, Icons.store_outlined);
      case NotBillingReason.ownerAbsent:
        return _ReasonMeta(
            'Absent', Colors.blue.shade600, Icons.person_off_outlined);
      case NotBillingReason.creditIssue:
        return _ReasonMeta(
            'Credit Issue', AppColors.error, Icons.credit_card_off_outlined);
      case NotBillingReason.noOrder:
        return _ReasonMeta(
            'No Order', AppColors.foregroundMuted, Icons.shopping_cart_outlined);
      case NotBillingReason.outOfStock:
        return _ReasonMeta(
            'Out of Stock', AppColors.amber, Icons.inventory_2_outlined);
    }
  }

  @override
  Widget build(BuildContext context) {
    final meta = _reasonMeta;
    return GestureDetector(
      onTap: widget.onTap,
      onTapDown: (_) => setState(() => _pressed = true),
      onTapUp: (_) => setState(() => _pressed = false),
          ...
          child: Row(
            children: [
              Container(
                ...
                child: Icon(meta.icon, size: 20.r, color: meta.color),
              ),
              SizedBox(width: 14.w),
              Expanded(
                child: Column(
                  ...
                  children: [
                    Row(
                      children: [
                        Text(
                          widget.notBilling.notBillingNumber,
                          style: GoogleFonts.barlowCondensed(
                            fontSize: 15.sp,
                            fontWeight: FontWeight.w800,
                            letterSpacing: 0.3,
                    ...
                    Row(
                      children: [
                        Text(
                          _formatDate(widget.notBilling.notBillingDate),
                          style: GoogleFonts.barlowCondensed(
                            fontSize: 12.sp,
                            fontWeight: FontWeight.w600,
                            color: AppColors.foreground,
...
class _ReasonMeta {
  ...
  final Color color;
  final IconData icon;
  const _ReasonMeta(this.label, this.color, this.icon);
}
