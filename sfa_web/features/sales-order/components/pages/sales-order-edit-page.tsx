...
export function SalesOrderEditPage({ orderId }: SalesOrderEditPageProps) {
  ...
  const { mutate: updateOrder, isPending, fieldErrors } = useUpdateSalesOrder(orderId)
  ...
  const onSubmit = (data: UpdateSalesOrderInput) => {
  ...
  if (order && order.status !== SalesOrderStatus.Draft) {
    return (
