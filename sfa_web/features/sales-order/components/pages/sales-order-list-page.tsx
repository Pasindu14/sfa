...
import { useSalesOrderStats } from '../../hooks/sales-order.hooks'
...
export function SalesOrderListPage() {
  ...
  const { fromDate, toDate } = useSalesOrderFilters()
  const { data: stats, isLoading: statsLoading } = useSalesOrderStats(fromDate, toDate)
    ...
    <div className="flex flex-col gap-6 p-6">
      ...
      <div className="flex items-center justify-between bg-muted/90 p-10 rounded-lg">
        <div>
          ...
          <p className="text-muted-foreground">Manage and track sales order workflow across all distributors.</p>
        </div>
