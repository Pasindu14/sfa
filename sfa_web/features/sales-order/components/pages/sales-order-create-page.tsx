...
import { useDefaultPricingStructure, useCreateSalesOrder, useSubmitSalesOrder } from '../../hooks/sales-order.hooks'
import {
...
export function SalesOrderCreatePage() {
  ...
  const { mutate: submitOrder } = useSubmitSalesOrder()
  ...
  const onSubmitForApproval = (data: CreateSalesOrderInput) => {
    createOrder(
      { ...data, items: data.items.filter((i) => i.productId > 0) },
      {
        onSuccess: (created) =>
          submitOrder(created.id, {
            onSuccess: () => router.push(`/sales-orders/${created.id}`),
          }),
      },
    )
  }
  ...
  const onSaveDraft = () => {
    ...
    createOrder(
      ...
      { onSuccess: (created) => router.push(`/sales-orders/${created.id}`) },
    )
  }
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[1fr_300px] gap-6 items-start">
            ...
            <div className="space-y-4 lg:sticky lg:top-6">
              ...
              <Button
                ...
                asChild
              >
                <Link href="/sales-orders">Discard &amp; Cancel</Link>
              </Button>
            </div>
          </div>
        </form>
