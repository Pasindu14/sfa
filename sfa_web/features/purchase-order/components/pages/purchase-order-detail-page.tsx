...
function OrderActionsPanel({ order }: { order: PurchaseOrderDto }) {
  ...
  const { mutate: repApprove, isPending: isRepApproving } = useRepApprovePurchaseOrder()
  const { mutate: approve, isPending: isApproving } = useApprovePurchaseOrder()
  const { mutate: reject, isPending: isRejecting } = useRejectPurchaseOrder(order.id)
  const { mutate: acknowledge, isPending: isAcknowledging } = useAcknowledgePurchaseOrder()
  const { mutate: finalize, isPending: isFinalizing } = useFinalizePurchaseOrder()
  const { mutate: cancel, isPending: isCancelling } = useCancelPurchaseOrder(order.id)
  ...
  return (
    <div className="space-y-2">
      ...
      {/* Terminal states */}
      {(status === PurchaseOrderStatus.Finalized || status === PurchaseOrderStatus.Cancelled) && (
        <p className="text-sm text-center text-muted-foreground py-2">
          {status === PurchaseOrderStatus.Finalized ? 'This order has been finalized.' : 'This order has been cancelled.'}
        </p>
      )}
    </div>
  )
    ...
    <div className="flex flex-col gap-6 p-4 sm:p-6 lg:w-3/4 mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 bg-muted/90 p-6 sm:p-10 rounded-lg">
        <div>
          ...
          <p className="text-muted-foreground text-sm sm:text-base">
            Purchase order · {order.distributorName} · Created {formatDateShort(order.createdAt)}
          </p>
        </div>
        <Button variant="outline" size="sm" onClick={() => router.back()} className="gap-2 self-start sm:self-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_340px] gap-6 items-start">
        ...
        <div className="space-y-4 lg:sticky lg:top-6">
          ...
          {/* Rejection Details */}
          {order.status === PurchaseOrderStatus.PendingDistributorAcknowledgement && rejectedEntry && (
