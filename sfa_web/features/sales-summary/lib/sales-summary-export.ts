...
export async function exportSalesSummaryExcel(data: SalesSummaryResponse): Promise<void> {
  ...
  const titleRow = ws.addRow(['Sales Summary'])
  titleRow.font = { bold: true, size: 14 }
  ws.addRow([subtitle(data)])
  if (!data.targetsAvailable && data.targetsUnavailableReason) {
    ws.addRow([`Targets not shown: ${data.targetsUnavailableReason}`])
  }
  ws.addRow([])
  ...
  for (const row of data.rows) {
    ws.addRow(
      columns.map((c) => {
        const v = c.get(row)
        // Blank, not 0 — a null target means "not measurable", and a zero would read as a miss.
        return v === null ? '' : v
      })
    )
  }
...
export function exportSalesSummaryPdf(data: SalesSummaryResponse): void {
  ...
  const head = columns
    .map((c) => `<th style="text-align:${c.align}">${esc(c.header)}</th>`)
    .join('')
  ...
  const totals = columns
    .map((c, i) => {
      ...
      const t = c.getTotal ? c.getTotal(data.totals) : null
      return `<td style="text-align:${c.align}">${c.getTotal ? fmt(t ?? null, c.money) : ''}</td>`
    })
    .join('')

  const note =
    !data.targetsAvailable && data.targetsUnavailableReason
      ? `<p class="note">${esc(data.targetsUnavailableReason)}</p>`
      : ''

  win.document.write(`<!DOCTYPE html><html><head><meta charset="utf-8" />
<title>Sales Summary — ${esc(data.from)} to ${esc(data.to)}</title>
<style>
  ...
  .note { color:#92400e; font-size:11px; margin:0 0 12px; }
  ...
  <h1>Sales Summary</h1>
  <p class="sub">${esc(subtitle(data))}</p>
  ${note}
  <table>
