...
export class FleetPage {
  ...
  constructor(readonly page: Page) {
    this.addButton = page.getByRole('button', { name: 'Add Fleet' })
    this.searchInput = page.getByPlaceholder('Search fleets...')
  ...
  async openCreateDialog() {
    ...
    await expect(this.page.getByRole('heading', { name: 'Create Fleet' })).toBeVisible()
  }

  async fillForm(name: string) {
    const dialog = this.page.locator('[role="dialog"]')
    await dialog.getByPlaceholder('Enter fleet name').clear()
    await dialog.getByPlaceholder('Enter fleet name').fill(name)
  }

  async submitCreateForm() {
    await this.page.getByRole('button', { name: 'Create Fleet' }).click()
  }

  async submitEditForm() {
    await this.page.getByRole('button', { name: 'Update Fleet' }).click()
  }
