...
/** Test data shape for creating / editing an area */
...
/**
 * Page Object Model for the /areas page.
 *
...
export class AreaPage {
  ...
  constructor(readonly page: Page) {
    this.addButton = page.getByRole('button', { name: 'Add Area' })
    this.searchInput = page.getByPlaceholder('Search areas...')
  ...
  /** Get a table row that contains the given area name */
  ...
  /** Assert the status badge text for a given area row */
  ...
  /** Open the Create Area dialog */
  async openCreateDialog() {
    await this.addButton.click()
    await expect(this.page.getByRole('heading', { name: 'Create Area' })).toBeVisible()
  }

  /** Fill the area form fields (works for both create and edit dialogs) */
  async fillAreaForm(data: Partial<AreaFormData>) {
    ...
    if (data.regionName !== undefined) {
      // RegionSelect wraps SelectTrigger in a <div>, so getByLabel resolves to
      // the wrapper — use getByRole('combobox') to target the actual trigger.
      // Also wait for it to be enabled: useActiveRegions() loads async and the
      ...
      await this.page.getByRole('option', { name: data.regionName }).click()
      await this.page.locator('[role="listbox"]').waitFor({ state: 'hidden', timeout: 3_000 }).catch(() => {})
    }
  }

  /**
   * Select the first available region option in the Region dropdown.
   * Waits for the async useActiveRegions() fetch to complete before clicking.
   */
  async selectFirstRegion() {
    const dialog = this.page.locator('[role="dialog"]')
    const selectTrigger = dialog.getByRole('combobox').first()
    // Wait for regions to load — trigger is disabled during the fetch
    await expect(selectTrigger).toBeEnabled({ timeout: 10_000 })
    await selectTrigger.click()
    await this.page.locator('[role="listbox"]').waitFor({ state: 'visible', timeout: 5_000 })
  ...
  async submitCreateForm() {
    await this.page.getByRole('button', { name: 'Create Area' }).click()
  }

  /** Click the submit button inside the edit dialog */
  async submitEditForm() {
    await this.page.getByRole('button', { name: 'Update Area' }).click()
  }
