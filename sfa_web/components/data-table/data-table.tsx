...
// ** import types
import type { Cell, CellContext, ColumnDef, ColumnFiltersState, ColumnResizeMode, Row, ExpandedState, VisibilityState } from "@tanstack/react-table";
...
// Extended column def for subrow columns that may have meta/accessorKey
type ExtendedColumnDef = ColumnDef<unknown, unknown> & {
  meta?: { title?: string };
  accessorKey?: string;
};
...
export interface SubRowsConfig<TData> {
  ...
  subRowColumns?: ColumnDef<unknown, unknown>[];
...
interface DataTableProps<TData extends ExportableData, TValue> {
  ...
  getSubRowColumns?: (handleRowDeselection: ((rowId: string) => void) | null | undefined) => ColumnDef<unknown, unknown>[];

  // Data fetching function
  fetchDataFn: ((params: DataFetchParams) => Promise<DataFetchResult<TData>>) |
               ((page: number, pageSize: number, search: string, dateRange: { from_date: string; to_date: string }, sortBy: string, sortOrder: string, caseConfig?: CaseFormatConfig, customFilters?: Record<string, unknown>) => unknown);
  ...
  renderCustomFilters?: (filters: Record<string, unknown>, setFilters: (filters: Record<string, unknown>) => void) => React.ReactNode;
...
export function DataTable<TData extends ExportableData, TValue>({
  ...
  const [dateRange, setDateRange] = useConditionalUrlState<{
    ...
    "dateRange",
    tableConfig.defaultDateRange ?? { from_date: "", to_date: "" },
  );
  ...
  >("columnFilters", []);
  const [customFilters, setCustomFilters] = useConditionalUrlState<Record<string, unknown>>(
    "customFilters",
  ...
  const getSortedSubrows = useCallback(
    (parentId: string, subrows: Record<string, unknown>[]) => {
      ...
      setSelectedItemIds((prev) => {
        ...
        if (subRowsConfig?.enabled) {
          ...
          dataItems.forEach((item) => {
            ...
            const subRowsData = (item as Record<string, unknown>)[
              subRowsConfig.subRowsField || "subRows"
            ];
            if (Array.isArray(subRowsData)) {
              subRowsData.forEach((subRow: Record<string, unknown>) => {
                const subRowId = String(subRow[idField as string]);
      ...
      if (Array.isArray(subRowsData)) {
        subRowsData.forEach((subRow: Record<string, unknown>) => {
          ...
          if (selectedItemIds[subRowId]) {
            ...
            const subRowItemId = subRow[idField as string];
            subrows.push({
              parentId:
  ...
  const getSelectedSubRows = useCallback(async (): Promise<TData[]> => {
    ...
    dataItems.forEach((item) => {
      ...
      if (Array.isArray(subRowsData)) {
        subRowsData.forEach((subRow: Record<string, unknown>) => {
          if (selectedSubrowIds.has(String(subRow[idField as string]))) {
  ...
  // Auto sign-out on unauthorized errors
  useEffect(() => {
    if (isError && error?.message === "You must be logged in") {
            ...
            {table.getHeaderGroups().map((headerGroup) => (
              <TableRow key={headerGroup.id}>
                {headerGroup.headers.map((header) => (
                  <TableHead
                    className="px-2 py-2 relative text-left group/th bg-muted [&:not(:last-child)]:border-r"
                    key={header.id}
                        ...
                        >
                          {subRowColumns.map((column, colIndex) => {
                            ...
                            const isSortable =
                              ...
                              col.id !== "actions" &&
                              col.id !== "expand";
                            const columnId = accessorKey || col.id || "";
                              ...
                              >
                                ...
                                typeof col.cell === "function"
                                  ? col.cell({
                                      ...
                                      row,
                                      column,
                                      cell: { getValue: () => value } as unknown as Cell<unknown, unknown>,
                                      table,
                ...
                return (
                  ...
                  >
                    {row.getVisibleCells().map((cell, cellIndex) => (
                      <TableCell
                        className="px-4 py-2 truncate max-w-0 text-left [&:not(:last-child)]:border-r"
                        key={cell.id}
