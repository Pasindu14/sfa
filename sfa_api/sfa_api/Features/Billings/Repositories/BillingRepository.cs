...
public class BillingRepository(AppDbContext db) : IBillingRepository
{
    ...
    public Task<Billing?> GetByIdAsync(int id, CancellationToken ct = default)
        => _db.Billings
              ...
              .Include(x => x.Items)
                  .ThenInclude(i => i.PricingStructure)
              .Include(x => x.PricingStructure)
        ...
        CancellationToken ct = default)
    {
        ...
        if (repStatus.HasValue)
            query = query.Where(x => x.RepStatus == repStatus.Value);

        if (distributorStatus.HasValue)
            query = query.Where(x => x.DistributorStatus == distributorStatus.Value);
