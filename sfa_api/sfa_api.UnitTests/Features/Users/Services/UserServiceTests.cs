...
public class UserServiceTests
{
    ...
    public async Task GetAllUsersAsync_ReturnsPaginatedList()
    {
        ...
        _repoMock.Setup(r => r.GetAllUsersAsync(0, 10, null, null, null, It.IsAny<CancellationToken>()))
    ...
    {
        _repoMock.Setup(r => r.GetAllUsersAsync(10, 10, null, null, null, It.IsAny<CancellationToken>()))
                 .ReturnsAsync((Enumerable.Empty<User>(), 0));

        await _sut.GetAllUsersAsync(2, 10);

        // skip = (page-1) * pageSize = (2-1) * 10 = 10
        _repoMock.Verify(r => r.GetAllUsersAsync(10, 10, null, null, null, It.IsAny<CancellationToken>()), Times.Once);
    }
    ...
    public async Task GetAllUsersAsync_WithSearch_PassesSearchToRepository()
    {
        ...
        _repoMock.Setup(r => r.GetAllUsersAsync(0, 10, search, null, null, It.IsAny<CancellationToken>()))
                 .ReturnsAsync((Enumerable.Empty<User>(), 0));

        await _sut.GetAllUsersAsync(page: 1, pageSize: 10, search: search);

        _repoMock.Verify(r => r.GetAllUsersAsync(0, 10, search, null, null, It.IsAny<CancellationToken>()), Times.Once);
    }

    [Fact]
    public async Task GetAllUsersAsync_WithIsActive_PassesFilterToRepository()
    {
        _repoMock.Setup(r => r.GetAllUsersAsync(0, 10, null, null, true, It.IsAny<CancellationToken>()))
                 .ReturnsAsync((Enumerable.Empty<User>(), 0));

        await _sut.GetAllUsersAsync(page: 1, pageSize: 10, isActive: true);

        _repoMock.Verify(r => r.GetAllUsersAsync(0, 10, null, null, true, It.IsAny<CancellationToken>()), Times.Once);
    }
