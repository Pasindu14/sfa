...
| 4 | **IsActive filter on every query** | `.Where(x => x.IsActive)` in every WHERE clause — `IsActive` is the universal soft-delete flag; DELETE also sets `IsDeleted = true` as audit trail |
...
- **`IsDeleted`** is the audit flag — `false` by default; set to `true` only by the DELETE endpoint to distinguish explicit deletion from deactivation
- **Deactivation:** set `IsActive = false` via `ExecuteUpdateAsync`
- **Deletion:** set `IsActive = false` AND `IsDeleted = true` via `ExecuteUpdateAsync`
